...
During the plotting step, it is possible to specify which groups of features or regions to add into the plot. The results of the \code{parseFeatures} or \code{parseRegions} can be used directly by the \code{plot.matrices} function.
...
Usage example:
\code{DF <- plot.matrices(list(group1=c("file1", "file2"), group2=(c("file3", "file4"), groups)))}

The \code{plot.matrices} function returns the \code{data.frame} that was used with ggplot2 to produce the results.
