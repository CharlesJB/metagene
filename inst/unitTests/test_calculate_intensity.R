...
test.getGenes_not_valid_specie<- function() {
	obs <- tryCatch(MetaFeatures:::getGenes(specie="tomato"), error=conditionMessage)
...
test.getGenes_human<- function() {
	data<-MetaFeatures:::getGenes("human")
...
test.getGenes_mouse<- function() {
	data<-MetaFeatures:::getGenes("mouse")
	checkEquals(names(data), c("feature", "strand", "space", "start_position", "end_position"),  "The returned dataset does not have the expected column names")
	checkTrue(length(data[, 1]) > 0, "The returned dataset has not observation.")
}
...
test.extractReadsInRegion_zero_start<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 0, 100), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
...
test.extractReadsInRegion_negative_start<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", -4, 100), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
	exp <- "The starting position has to be a positive integer."
	checkIdentical(obs, exp, msg="A negative maximum distance argument did not generate an exception with expected message.")
}
...
test.extractReadsInRegion_not_integer_start<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 2.33, 100), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
	exp <- "The starting position has to be a positive integer."
	checkIdentical(obs, exp, msg="A decimal maximum distance argument did not generate an exception with expected message.")
}
...
test.extractReadsInRegion_not_integer_start<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", "NotAInteger", 100), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
	exp <- "The starting position has to be a positive integer."
...
test.extractReadsInRegion_zero_end<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 10, 0), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
...
test.extractReadsInRegion_negative_end<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 3, -100), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
exp <- "The ending position has to be a positive integer."
...
test.extractReadsInRegion_not_integer_end<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 3, 33.2), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
	exp <- "The ending position has to be a positive integer."
...
test.extractReadsInRegion_not_integer_end<- function() {
	temp_file<-tempfile()
	file.create(temp_file)
	obs <- tryCatch(MetaFeatures:::extractReadsInRegion(temp_file, "X", 20, "NotAInteger"), error=conditionMessage, finally={if (file.exists(temp_file)){file.remove(temp_file)}})
	exp <- "The ending position has to be a positive integer."
