...
bamFile2Rep1 <- system.file("extdata/align2_rep2.bam", package="metagene")
...
* One column per group of files (replicates and/or controls). The column can take only three valid values: 
	+ 0: ignore file
	+ 1: input
	+ 2: control
...
It is possible to provide, to metagene package, a text file that contains a list of Ensembl Gene ID(s) as input. The files must be formated in the following way:
